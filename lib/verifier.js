...
export async function getBaseIds() {
  ...
  let nftsBaseId, eggsBaseId;

  if (FIXED_NFTS_BASE_ID !== null && FIXED_EGGS_BASE_ID !== null) {
    nftsBaseId = FIXED_NFTS_BASE_ID;
    eggsBaseId = FIXED_EGGS_BASE_ID;
  } else {
    ...
    nftsBaseId = BigInt(result.nftsBaseId ?? result[3]);
    eggsBaseId = BigInt(result.eggsBaseId ?? result[4]);

    if (FIXED_NFTS_BASE_ID !== null) nftsBaseId = FIXED_NFTS_BASE_ID;
    if (FIXED_EGGS_BASE_ID !== null) eggsBaseId = FIXED_EGGS_BASE_ID;
  }

  const value = { nftsBaseId, eggsBaseId };
...
export function isPet(tokenId, baseIds) {
  ...
  return id >= baseIds.eggsBaseId && id < baseIds.nftsBaseId;
}
